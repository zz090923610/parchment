from src import JobHdl, ConfigHDL, MarkDownGenerator
...
def add_alias(alias_path, alias):
    with open(alias_path, mode='a', encoding='utf-8') as a_file:
        a_file.write(alias)


class FileOperator(object):
    def __init__(self):
        self.cfg_hdl = ConfigHDL()
    ...
    def dump_daily_digest(self):
        ...
        os.chdir(self.cfg_hdl.job_root)
        file_name_list = glob.glob('*.xml')
        digest_path = os.path.join(self.cfg_hdl.digest_path, 'DailyDigest' + str(date_today) + '.md')
        generator = MarkDownGenerator(os.path.expanduser(digest_path),date_today)
        for a_file in file_name_list:
            job_path = os.path.join(self.cfg_hdl.job_root, a_file)
            entry = JobHdl()
            entry.read_from_xml(job_path)
